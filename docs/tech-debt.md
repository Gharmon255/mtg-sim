...
- Smothering Tithe-style opponent draw triggers now have heuristic pay-or-treasure decisions, but they do not model exact payment timing or all draw-tax variants.
- Rhystic Study, Mystic Remora, and Esper Sentinel-style opponent-cast triggers now have heuristic pay-or-draw decisions, but they do not model exact tax payment timing/rules; "declined" tax metrics mean unpaid by heuristic rather than an exact player choice, Esper Sentinel may still use a simplified payment fallback when exact power is unavailable, and Mystic Remora cumulative upkeep is not modeled by this trigger path.
